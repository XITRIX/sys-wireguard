...
- The attempted transport de-serialization slice in `sdk/src/switch_transport.cpp` regressed stability first with an immediate host-open crash in the C++ `thread_local` variant, then with `STAGE_RTSP_HANDSHAKE` failures in the explicit per-thread handle-cache variant. The current build is intentionally rolled back to the previously stable single `swg:ctl` service handle guarded by one mutex so streaming can recover before transport parallelism is revisited.
...
- The in-house WireGuard handshake/transport implementation has been replaced with the BSD-licensed `smartalock/wireguard-lwip` protocol core as a git submodule under `third_party/wireguard-lwip`; SWG now builds a local `wireguard_lwip_core` target and keeps `common/src/wg_handshake.cpp` as the adapter boundary.
- The unused local WireGuard keypair-generation helper left over from the in-house protocol backend has been removed; `wg_crypto` now remains only for config preflight and deterministic test support.
- Added `docs/wireguard-backend.md` with AGENTS-style intent, touched files, constraints, implementation, verification, and follow-up tasks for the backend swap.
...
- Re-run Moonlight-Switch on-device with the rebuilt `Moonlight.nro` and confirm the rollback restores host-open stability plus RTSP handshake/stream start.
- Once streaming is restored, return to the still-active input-latency path in `sdk/src/enet_compat.cpp` and `sdk/src/compat_bridge_switch.cpp` instead of retrying multi-handle `swg:ctl` transport changes immediately.
