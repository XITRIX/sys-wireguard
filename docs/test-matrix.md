...
| BSD UDP adapter lab build (`dbg0715i`) | Build verified; hardware pending | Normal Switch, adapter-lab Switch, and host presets build; `swg_tests` passes with loopback socket access. `dbg0715i` retains the hardware-proven Poll repair and adds post-MMsg forced-nonblocking direct UDP receives, 1 ms active-media Poll/Select slices, source-port telemetry, and per-session nonblocking counts. |
| Adapter-lab Moonlight video regression | `dbg0715h` reached media and received eight video datagrams; retest `dbg0715i` | The repaired RTSP Poll was followed by two 52-byte receives, successful MMsg, and eight 1408-byte video receives. A separate direct UDP `RecvFrom` blocked the sole worker for `99086 us`; require `dbg0715i` to progress beyond 8/16/32 packets without a comparable slow forward. |
| Adapter-lab Moonlight disconnect regression | Media reached; retest continuous stream/disconnect on `dbg0715i` | `dbg0715h` reached media but the video socket closed before sustained playback. Confirm Moonlight remains responsive, issues normal `Close`/`Shutdown` during disconnect, and every adapter session closes with `unsupported=0`. |
| Adapter-lab graceful stop / `bsdsocket` crash regression | Hardware verified on `dbg0714a`; reconfirm on `dbg0715i` | The supplied `dbg0714a` log completed both adapter shutdown stages and both hook uninstalls without a new `0100000000000012` crash. Re-run after the media scheduling change to guard against regression. |
