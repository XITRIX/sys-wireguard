...
| BSD UDP adapter lab build | Build verified | `build/switch-debug-bsd-adapter-lab` compiles with `SWG_ENABLE_EXPERIMENTAL_BSD_MITM_ADAPTER_LAB=ON`; it now preserves the BSD `RegisterClient` transfer-memory handle, asks original `bsd:u` to allocate fd numbers, handles tracked IPv4 UDP sockets through tunnel datagram handles, and proxies policy-approved local/multicast IPv4 UDP plus local IPv4 TCP through direct native BSD sockets, with hardware validation pending |
