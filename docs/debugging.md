...
When `SWG_ENABLE_EXPERIMENTAL_BSD_MITM_ADAPTER_LAB=ON`, build marker `dbg0715i` should show `installed adapter lab MitM handles for bsd:u`, `active bsd:u MITM adapter lab ready`, `direct_backend=original_bsd_forwarding`, `session_scheduler=control_priority_then_media_round_robin`, `direct_wait_slice_ms=1`, `wait_slicing=media_phase_only`, `media_udp_receive=force_dontwait`, `media_phase_trigger=successful_mmsg`, `poll_outcome=hipc_shape_ready_repair`, `poll_ready_repair=unparseable_response_with_new_ready_descriptors_only`, `poll_select_timeout=exact_control_cooperative_media`, `mmsg_direct_forward=route_validated`, `mmsg_logging=sparse`, `private_bsd_runtime=disabled`, `shutdown_guard=park_on_live_worker`, `title_selector=exact_allowlist`, and `selected` increasing for `bsd:u`. Before media, requested and forwarded wait timeouts must match. The first successful MMsg should log `scheduler_phase=media_round_robin wait_slicing=enabled`; later direct UDP receives should show forced nonblocking flags and long waits should use the 1 ms cooperative limit.
...
If a LAN route logs `-> direct` but no `forwarded bsd:u Connect direct ... to original service` line follows, the request failed before the raw forward or the adapter lost its cloned original session. Check the preceding `Socket`, clone, route-plan, and forward failure lines. Nonblocking `EINPROGRESS`, `EWOULDBLOCK`/`EAGAIN`, and `EALREADY` responses are returned unchanged to the app.

If Moonlight cannot reach media setup, compare the RTSP/control sequence with the scheduler phase. `dbg0715h` proved the narrow exceptional Poll repair works: `ready_result_repaired=true` was followed by port-48010 receives and successful MMsg/media setup. If `dbg0715i` does not reproduce that sequence, preserve the HIPC/CMIF shape fields, all input/output readiness fields, and the next socket operation.

If media begins but video stops, inspect direct UDP receive logs after `scheduler_phase=media_round_robin`. `dbg0715i` should report `media_forced_nonblocking=true`; `RecvFrom` should also identify a nonzero `source_port`, and `media_nonblocking_recv_count` plus sparse packet counts should grow beyond 8/16/32. Any post-media direct UDP receive near 100 ms, a count that stops growing, repeated unexpected errno values, or a zero source port on successful `RecvFrom` is the next key diagnostic.

If stopping SWG produces an Atmosphère crash report for process `bsdsocket` (`0100000000000012`) with a user break, first verify the build marker is `dbg0715i`. This build does not start the adapter's private `BsdSocketRuntime` for direct traffic, forwards an app-requested `Close` exactly once, and relies on original-session closure for forced fd cleanup. A passing manager stop logs `adapter shutdown stage: begin`, `client sessions released`, `adapter shutdown stage: complete`, `active bsd:u MITM adapter lab stopped`, then both hook-uninstall lines. If any worker fails to join, the expected fail-safe is `unsafe shutdown refused` followed by a parked sysmodule requiring reboot; process exit and hook uninstall are intentionally skipped.
...
If Moonlight creates a TCP socket and then logs `Curl error: Could not connect to server`, check the bootstrap and socket lines. The adapter should report `original=registered`, pointer-buffer query `size=0x0`, `forward=cloned`, `socket_response=forward_exact_fd0_patch`, `backend=forwarded-original-bsd`, and `domain=2`; fd zero remains valid and may be patched to a replacement original-service fd. `Fcntl`, socket options, direct `Connect`, and later TCP send/receive now stay on that original service. If Curl stops before the forwarded `Connect`, use the bounded dispatch traces to identify the request where it stopped.
...
If HOS crashes while Moonlight reaches host-status ping and the sysmodule crash symbols land in `BsdMitmAdapterServer::HandleGetSockName`, verify the marker. `dbg0715i` forwards direct name queries and their original HIPC buffers unchanged to the original BSD service; it no longer bounces this path through a private socket or adapter-owned scratch buffer.
