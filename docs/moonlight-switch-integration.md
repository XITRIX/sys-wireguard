...
- `Client::OpenAppSession()` / `CloseAppSession()` / `GetNetworkPlan()` / `SendPacket()` / `RecvPacket()`
- `AppSession::SendPacket()` / `ReceivePacket()` for moving authenticated payloads once tunnel traffic is flowing
...
The new packet API is intentionally narrow: it gives a Moonlight-side shim one place to push and pull authenticated tunnel payloads without requiring transparent routing first, but it still does not replace Moonlight's existing socket semantics by itself.
