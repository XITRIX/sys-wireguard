...
- The live connect path now resolves IPv4 endpoints, validates the handshake, schedules authenticated keepalives, and accepts inbound authenticated transport packets into a bounded internal queue, but that queue is still not surfaced through `swg:ctl` or the SDK, and there is still no endpoint roaming support or retransmit/backoff policy beyond the current bounded handshake retry.
