...
- The live connect path now resolves IPv4 endpoints, validates the handshake, schedules authenticated keepalives, and surfaces authenticated transport send/receive through `swg:ctl`, but reconnect recovery is still limited to outbound send failures and there is still no endpoint roaming support or broader sustained transport loop.
