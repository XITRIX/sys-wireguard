...
- The new Switch integration app now exposes real `ResolveDns()` and sample `SessionSocket` checks on hardware, but its send/receive controls are still diagnostic until the SDK grows a stronger Moonlight-ready transport shim.
...
- The app API now includes policy-safe DNS responses plus a route-aware socket wrapper for direct, denied, and tunnel-guided cases, but the tunnel DNS path is still limited to synchronous IPv4 A-record lookups and native TCP-over-tunnel or transparent socket interception are still deferred.
