...
- The live connect path now resolves IPv4 endpoints, validates the handshake, schedules authenticated keepalives, and accepts inbound authenticated keepalives from the validated endpoint, but it still has no general transport RX/TX loop, no authenticated payload handling beyond empty keepalives, no endpoint roaming support, and no retransmit/backoff policy beyond the current bounded handshake retry.
